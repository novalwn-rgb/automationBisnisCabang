...
// Script Dibawah Menampilkan Full Row
...
String excelPath = localDir + "/" + latestFile
String targetSubmission = "BN57/2113/UND/FR-0101/JKT1/01/26"
...
boolean found = false
int foundExcelRow = -1        // row Excel (1-based)
String dataOrder = ""         // nomor urut di tabel (kolom No)

for (int i = 1; i <= sheet.getLastRowNum(); i++) { // mulai dari row data

	Row row = sheet.getRow(i)
	if (row == null) continue

	Cell submissionCell = row.getCell(2) // kolom Submission Number
	if (submissionCell == null) continue

	String submissionValue = submissionCell.toString().trim()

	if (submissionValue.equalsIgnoreCase(targetSubmission)) {

		found = true

		// 🔢 Row Excel (user-friendly)
		foundExcelRow = i + 1

		// 🔢 Nomor urut tabel (kolom No)
...
assert found : "Submission Number tujuan TIDAK ditemukan di Excel: " + targetSubmission

KeywordUtil.markPassed(
	"Submission Number [" + targetSubmission + "] ditemukan | " +
	"Row Excel: " + foundExcelRow +
